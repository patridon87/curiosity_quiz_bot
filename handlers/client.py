...
async def question(message: types.Message, state: FSMContext):
    ...
    if data['question'] is None:
        ...
        buttons = [InlineKeyboardButton(text=data['question'][4],
                                        callback_data=1),
                   InlineKeyboardButton(text=data['question'][5],
                                        callback_data=2),
                   InlineKeyboardButton(text=data['question'][6],
                                        callback_data=3),
                   InlineKeyboardButton(text=data['question'][7],
                                        callback_data=4)]
        ...
        if photo == 'NULL':
            await bot.send_message(message.from_user.id,
                                   ...
                                   f'{text}', reply_markup=inline_keyboard)
        else:
...
def register_handlers_client(dp: Dispatcher):
    ...
    dp.register_message_handler(question, commands=['Получить_вопрос'],
                                content_types=['text'],
                                state=None)
    dp.register_callback_query_handler(answer_check, text=[1, 2, 3, 4],
